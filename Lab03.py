...
dt = dt.sort_values(["Max Salary", "Min Salary"])
...
notNullSalary = dt.loc[(dt['Max Salary'].notnull()) & (dt['Min Salary'].notnull())]
salariesGroups = np.array_split(notNullSalary['Min Salary'].unique(),10)
lastSalary = 0
daysRows = []
FolderCount = 0
...
for salaryGroup in salariesGroups:
    FolderCount += 1
    group = notNullSalary.loc[(notNullSalary['Min Salary'] > lastSalary) & (notNullSalary['Min Salary'] <= max(salaryGroup))]
    ...
    if len(skills) != 0 :
        ...
        skills["Key skills"] = skills["Key skills"].apply(lambda x: x.split(';'))
        skills = skills.explode("Key skills")
        saveCount(skills, "Key skills", FolderName + "/KeySkills.csv")
...
print("********Часть 3********")
saveCount2(dt, "Name", "Min Salary", "Part 3 - Min Salary.csv")
saveCount2(dt, "Name", "Max Salary", "Part 3 - Max Salary.csv")
...
skills["Key skills"] = skills["Key skills"].apply(lambda x: x.split(';'))
skills = skills.explode("Key skills")
saveCount2(skills, "Name", "Key skills", "KeySkills.csv")
